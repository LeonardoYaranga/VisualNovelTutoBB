...
func handle_ui_set_layout(params: Dictionary) -> Dictionary:
	...
	var grow_h: String = params.get("grow_horizontal", "")
	var grow_v: String = params.get("grow_vertical", "")
	var grow_direction: String = params.get("grow_direction", "")
	# Map friendly names to Godot 4.x GrowDirection enum
	var dir_map = {
		"begin": Control.GROW_DIRECTION_BEGIN,
		"end":   Control.GROW_DIRECTION_END,
		"both":  Control.GROW_DIRECTION_BOTH,
		# legacy aliases
		"left":  Control.GROW_DIRECTION_BEGIN,
		"up":    Control.GROW_DIRECTION_BEGIN,
		"right": Control.GROW_DIRECTION_END,
		"down":  Control.GROW_DIRECTION_END,
	}
	if grow_direction != "" and dir_map.has(grow_direction):
		ctrl.grow_horizontal = dir_map[grow_direction]
		ctrl.grow_vertical   = dir_map[grow_direction]
	if grow_h != "" and dir_map.has(grow_h):
		ctrl.grow_horizontal = dir_map[grow_h]
	if grow_v != "" and dir_map.has(grow_v):
		ctrl.grow_vertical   = dir_map[grow_v]
